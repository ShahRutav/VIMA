...
from .xattn import XAttention
from ..hf_lm.gpt import Block
...
class XAttnGPT(OpenAIGPTPreTrainedModel):
    ...
    def forward(
        ...
        *,
        ...
        self._check_input(
            obs_action_tokens, prompt_tokens, prompt_mask, batch_first, obs_action_masks
        )
        if batch_first:
    ...
    def get_optimizer_groups(self, weight_decay, lr_layer_decay, lr_scale=1.0):
        gpt_pg, gpt_pids = transformer_lr_decay_optimizer_groups(
            self,
            layer_0_params=None,
        ...
        xattn_pg, xattn_pids = transformer_lr_decay_optimizer_groups(
            self,
            layer_0_params=None,
        ...
        other_pg, other_pids = default_optimizer_groups(
            self,
            weight_decay=weight_decay,
            lr_scale=lr_scale,
            no_decay_filter=[
            ...
            exclude_filter=lambda name, p: id(p) in gpt_pids + xattn_pids,
        )
        return gpt_pg + xattn_pg + other_pg, gpt_pids + xattn_pids + other_pids
