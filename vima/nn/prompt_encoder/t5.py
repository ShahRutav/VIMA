...
from .t5_with_adapter import T5EncoderModelWithAdapter
...
class T5PromptEncoder(nn.Module):
    def __init__(
        ...
        t5_model: str,
        *,
        unfreeze_last_n_layers: int = 0,  # 0 means freeze all
        ...
        ), "TODO: support methods `get_optimizer_groups` and `freeze_except_last_layers` for t5 with adapters"
        ...
        if adapter_type is None:
            ...
            transformer_freeze_except_last_layers(
                ...
                block_sequence_name="encoder.block",
                num_last_layers=unfreeze_last_n_layers,
            )
            if unfreeze_last_n_layers == 0:
                U.freeze_params(self.t5.encoder.final_layer_norm)
        else:
            self.t5 = T5EncoderModelWithAdapter.from_pretrained(
                t5_model,
    ...
    def forward(
        ...
        *,
        ...
        """
        self._check_input(x, attention_mask, batch_first)
        if batch_first:
            B, L, E = x.shape
        else:
            L, B, E = x.shape
    ...
    @U.call_once
    def _check_input(
        self, x, attention_mask: torch.Tensor | None = None, batch_first: bool = False
    ):
        assert x.dim() == 3
        ...
        if attention_mask is not None:
            assert attention_mask.shape == (B, L) or attention_mask.shape == (
                B,
                1,
                L,
            ), f"Expect `attention_mask` to have shape of either ({B, 1, L}) or ({B, L}), but got {attention_mask.shape}"
